...
// dump($_REQUEST); asdasdasdasdasdasdasddddd
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST")
    {
        ...
        elseif($_POST["action"] == "modal_employee"):
            ...
            $settings = query("select * from payroll_pei_settings");
                        ...
                        <div class="form-group">
                            ...
                            if($pa["deduction_type"] != ''):
                                ...
                                foreach($settings as $row):
                                        $hint = $hint . '<option value="'.$row["length_of_service"].'">'.$row["length_of_service"] . '</option>';
                                endforeach;
                                $hint = $hint . '
                            </select>
                        </div>
        ...
        elseif($_POST["action"] == "update_employee"):
            ...
            if(!empty($_POST["pro_rated_length"])):
                $settings = query("select * from payroll_pei_settings where length_of_service = ? ", $_POST["pro_rated_length"]);
            endif;
            ...
            if(!empty($settings)):
                ...
                $length = $settings[0]["length_of_service"];
            endif;
            ...
            query("update tblemployee set 
                    ...
                    ");
            query("update payroll_pei set  length_of_service = ?, percentage = ? where
                    employee_id = ? and payroll_id = ?",  $length, $percentage,$_POST["employee_id"], $_POST["payroll_id"]);
        ...
        elseif($_POST["action"] == "modal_gs"):
            ...
            elseif($_POST["action"] == "delete_employee"):
                // dump($_POST);
                query("delete from payroll_pei where employee_id = ?
                        and payroll_id = ?", $_POST["employee_id"], $_POST["payroll_id"]);
                $res_arr = [
                    "message" => "Successfully Processed",
                    "status" => "success",
            ...
            elseif($_POST["action"] == "add_all_employees"):
                ...
                $pg = $pg[0];
                $Pa = [];
                $pa = query("select employee_id from payroll_pei where year = ?", $pg["year"]);
                foreach($pa as $row):
               ...
               if(!empty($Include)):
                ...
                endforeach;
                $query = implode( ",", $inserts );
                query('insert into payroll_pei(payroll_id, employee_id, year) VALUES '.$query);
               endif;
            ...
            elseif($_POST["action"] == "add_employee"):
                ...
                $pg = query("select * from payroll_group where barcode = ?", $_POST["barcode"]);
                $pg = $pg[0];
                $query_string = "insert into payroll_pei 
                ...
                elseif($_POST["action"] == "save_payroll"):
                    ...
                    foreach($pa as $row):
                        ...
                        if(!empty($row["length_of_service"])):
                    ...
                    (
                        ...
                        lbp_number, pei, length_of_service,percentage, accrued_pei
                    ) VALUES ' . $query;
                    //dump($query_string);
                    query("delete from payroll_pei where payroll_id = ?", $pg["payroll_id"]);
                    query($query_string);
                    query("update payroll_group set payroll_status = 'done'
                        ...
                        $res_arr = [
                            "message" => "Successfully Added",
                            "status" => "success",
                            "link" => "pei?action=done&barcode=".$pg["barcode"],
                            ];
                            echo json_encode($res_arr); exit();
                    ...
                    elseif($_POST["action"] == "revert_payroll"):
                        ...
                        $res_arr = [
                            "message" => "Successfully Added",
                            "status" => "success",
                            "link" => "pei?action=barcode&barcode=".$_POST["barcode"],
                            ];
                            echo json_encode($res_arr); exit();
                ...
                elseif($_POST["action"] == "modal_others_done"):
                    ...
                    elseif($_POST["action"] == "pacs"):
                        ...
                        $string = $row["lbp_number"] . $row["LastName"] . ", " . $row["FirstName"] . " " . $row["MiddleName"][0] . ".";
                            $amount = to_findes_amount($row["accrued_pei"]);
                        ...
                        elseif($_POST["action"] == "generate_excel"):
                            ...
                            $d = $department[0];
                            $payroll_actual = query("select e.Employeeid, Gender, concat(e.LastName, ' ', e.NameExtension, ', ', e.FirstName) as fullname, p.* from payroll_pei p
                                                        left join tblemployee e
                            ...
                            if($_POST["form_type"] == "standard"):
                                $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("file_folder/payroll/pei/pei_default.xlsx");
                            elseif($_POST["form_type"] == "otc"):
                                $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("file_folder/payroll/pei/pei_pro_rated.xlsx");
                            elseif($_POST["form_type"] == "separated"):
                                $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("file_folder/payroll/pei/year_end_separated.xlsx");
                            elseif($_POST["form_type"] == "pro_rated"):
                                $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load("file_folder/payroll/pei/pei_pro_rated.xlsx");
                            endif;
                            ...
                            foreach($payroll_actual as $p):
                                ...
                                if($_POST["form_type"] == "standard"):
                                    ...
                                    $sheet->setCellValue('G'.$start_row ,$p["accrued_pei"]);
                                ...
                                elseif($_POST["form_type"] == "otc"):
                                    ...
                                    $sheet->setCellValue('D'.$start_row, $female);
                                    $sheet->setCellValue('E'.$start_row, $p["position_title"]);
                                    $sheet->setCellValue('G'.$start_row ,$p["pei"]);
                                    $sheet->setCellValue('I'.$start_row ,$p["length_of_service"]);
                                    $sheet->setCellValue('J'.$start_row ,$percentage);
                                    $sheet->setCellValue('L'.$start_row ,"=ROUND(G".$start_row."*J".$start_row.",2)");
        ...
        if(isset($_GET["action"])):
            if($_GET["action"] == "barcode"):
                ...
                elseif($_GET["action"] == "done"):
                    ...
                    select pa.*, e.FirstName, e.LastName from payroll_pei pa
                    left join tblemployee e
                    on pa.employee_id = e.Employeeid
                    where payroll_id = ?
                    order by e.LastName ASC, e.FirstName ASC
                    ", $pg["payroll_id"]);
                    // dump($payroll_actual);
                    render("apps/payroll/payroll_special_app/pei/pei_done.php", 
                    [
                        "title" => "Payroll Done", 
            ...
            $payroll_group = query("select pg.*, d.DeptCode from payroll_group pg
                                    ...
                                    on d.Deptid = pg.department_fund
                                    where 
                                    payroll_design = 'special' and payroll_type = 'PEI'
                                    order by year desc");
            // dump($payroll_group);
            render("apps/payroll/payroll_special_app/pei/pei_list.php", 
                ["title" => "PEI PAYROLL",
